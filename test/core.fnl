...
(fn test-destructuring []
  ...
  (assert-iife (local (x y) (values :x (do :y)))))
...
(fn test-pick-values []
  ...
  (== (let [ret [(pick-values 3 :x :y (do :z) :A (do :B))]]
        [ret (length ret)])
    [[:x :y :z] 3])
  (assert-iife [(pick-values 3 :x :y (do :z) :A (do :B))])
  ...
  (== (let [pack (or table.pack #(doto [$...] (tset :n (select :# ...))))]
      ...
      {1 :x 2 :y 3 :z :n 5})
  (assert-iife (pick-values 2 (do 1))
               "This is now an IIFE after reverting Xero's values patch")
  (assert-iife (pick-values 1 (do 1))
               "This is also now an IIFE after reverting Xero's values patch")
  (assert-no-iife (do (fn f [] (values :x :y)) (pick-values 1 (f)))
                  "This still has no IIFE, as there is no `do`"))
